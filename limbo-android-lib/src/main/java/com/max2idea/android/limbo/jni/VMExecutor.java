...
public class VMExecutor {
    ...
    public void prepareParams() throws Exception {
        anbuidata = this.context.getSharedPreferences("adata", Activity.MODE_PRIVATE);
...
//        paramsList.add("32M"); //Don't increase it crashes
    ...
    private void addCpuBoardOptions(ArrayList<String> paramsList) {
        ...
        if (anbuidata.getString("dualm", "").length() == 1) {
            mem1 = memory + 2;
            paramsList.add(this.memory + "M,slots=2,maxmem=" + mem1 + "M");
        } else {
    ...
    private void addBootOptions(ArrayList<String> paramsList) {
        if (this.bootdevice != null) {
            paramsList.add("-boot");
            if (anbuidata.getString("bootmenu", "").length() == 1) {
                paramsList.add(bootdevice + ",menu=on");
            } else {
    ...
    public void addRemovableDrives(ArrayList<String> paramsList) {
        ...
        if (anbuidata.getString("iommu", "").length() == 1 && Double.parseDouble(anbuidata.getString("mmachine", "")) > 25) {
            paramsList.add("-device");
            paramsList.add("intel-iommu");
        }

        if (anbuidata.getString("vmem", "").length() == 1) {
            paramsList.add("-device");
            paramsList.add("virtio-balloon-pci");
        }

        if (anbuidata.getString("vgpu", "").length() == 1) {
            paramsList.add("-device");
            paramsList.add("virtio-gpu-pci");
        }

        if (anbuidata.getString("vserial", "").length() == 1) {
            paramsList.add("-device");
            paramsList.add("virtio-serial-pci");
        }

        if (anbuidata.getString("vrng", "").length() == 1) {
            paramsList.add("-device");
